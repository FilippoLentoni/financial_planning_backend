...
It also stores a `market-data-batch` item for the run, then creates a `model-input` item with `source=yfinance-market-data`, and finally runs `run_math_model(input_id)` to create a `portfolio-optimization` output.
