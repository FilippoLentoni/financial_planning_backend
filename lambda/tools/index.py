...
def get_model_input(args: dict[str, Any]) -> dict[str, Any]:
...
def get_model_output(args: dict[str, Any]) -> dict[str, Any]:
    run_id = str(args.get("run_id") or args.get("simulation_id") or "").strip()
    run = _require_model_run(run_id)
...
def override_input(args: dict[str, Any]) -> dict[str, Any]:
...
def get_model_formulation(args: dict[str, Any]) -> dict[str, Any]:
    run_id = str(args.get("run_id") or args.get("simulation_id") or "").strip()
    run = _require_model_run(run_id)
...
def record_override(args: dict[str, Any]) -> dict[str, Any]:
    input_id = str(args.get("input_id") or "").strip()
    justification = str(args.get("justification") or "").strip()
    if not justification:
        raise ValueError("justification is required")
    _require_model_input(input_id)
    override_id = _stable_id("override", {"input_id": input_id, "justification": justification})
    item = {
        "id": override_id,
...
TOOL_HANDLERS = {
    ...
    "get_model_formulation": get_model_formulation,
